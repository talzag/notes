...
use Log;
use Session;
use Auth;use Cookie;use Request;use Route;
use View;
use Note;
use Response;use Input;
use DateTime;Use DateTimeZone;
use Parsedown;
use Google_Client;use Google_Service_Drive;use Gdoc;
...
class NotesController extends Controller {
	...
	private function handle_google_login() {
        ...
        return redirect($redirect_uri."?note=".Input::get("state")."&edit=true&gdoc=true");
    }
    ...
    private function handle_google_note_add($note,$note_id) {
		...
		$root = (!empty($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'];
		return redirect($root."?note=".$note_id."&edit=true&gdoc_added=true");
    }
